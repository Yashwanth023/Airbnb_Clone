...
import { toast } from "react-toastify"; import "../../CSS/Login.css";
...
const Login = () => {
    ...
    const submitHandler = (e) => {
        e.preventDefault(); dispatch(getLogIn({ email, password }));
    };
    useEffect(() => {
        if (errors && errors.length > 0) {
            ...
            navigate("/"); toast.success("User has logged successfully");
        }
    }, [isAuthenticated, errors, navigate]);
    return (<Fragment> <div className="row wrapper"> {loading && <LoadingSpinner />}
        {!loading && (<div className="col-10 col-lg-5">
