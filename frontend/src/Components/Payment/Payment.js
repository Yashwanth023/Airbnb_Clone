...
const Payment = () => {
  ...
  const handleSubmit = processPayment({
    ...
    stripe,elements,checkinDate,checkoutDate,propertyName,address,
    maximumGuest,nights,bookingId,propertyId,dispatch,navigate
  })
      ...
      <div className='col-10 col-lg-5'>
        {isAuthenticated ? (
          <form className='shadow-lg' onSubmit={handleSubmit}>
            ...
            <button className='paymentbtn' type='submit'>Pay - {totalPrice}</button>            
          </form>
        ) : (
          <div>
            {navigate("/login")}
